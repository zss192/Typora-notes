...
- 每个结点最多有m个分支(树的分叉)，若不是叶子结点的根结点最少有两个分支
- 除根结点外的所有非终端结点最少有(m/2)~上限~个分支
- 有n个分支的结点有n-1个关键字递增从左到右排列
- 叶子结点(失败结点)在同一层可用空指针表示，是查找失败到达的位置

8.B-树的查找    (类似于折半查找，但是可以有三个或多个方向)

如查找关键字42。首先在根结点查找，因为42>30，则沿着根结点中指针p\[1](下标从0开始)往左下走;因为39<42<45,则沿着子树根结点中指针p[1]往下走，在下层结点中查找关键字42成功，查找结束。
...
负载(装载因子)：表中结点/表的空间，所以当表越满越容易发生冲突
...
4.![image-20200612221437828](/home/zss/.config/Typora/typora-user-images/image-20200612221437828.png)

答：装填因子越大就越满越可能发生冲突。冲突少减少不必要的查找。

不能完全避免聚集(不是同义词却抢相同地址)只能减少
...
> 因为最多可查8个元素，所以查找成功时分母为8，分子就是每个元素查找的次数之和
>
